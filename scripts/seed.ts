...
function getRandomStatus(): OrderStatus {
  ...
  if (rand < 0.75) return 'paid';
  if (rand < 0.90) return 'cancelled';
...
function generateBatch(batchIndex: number, count: number): OrderDocument[] {
  const batch: OrderDocument[] = [];
...
export async function runSeed(): Promise<void> {
  ...
  for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
    ...
    if ((batchIndex + 1) % 5 === 0 || batchIndex === totalBatches - 1) {
      ...
      console.log(
        `[seed] Progress: ${insertedCount.toLocaleString()} / ${TOTAL_ORDERS.toLocaleString()} (${percent}%) [${elapsed}s]`
      );
    }
  }
