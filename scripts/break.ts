...
export async function runBreak(): Promise<void> {
  console.log('==================================================');
  ...
  // Safety check
  if (
    (!isSafeUrl(CHECKOUT_URL) || !isSafeUrl(PAYMENT_PROVIDER_URL)) &&
  ...
  try {
    ...
    if (!healthCheckout.ok) throw new Error(`Checkout health check returned ${healthCheckout.status}`);
    const healthProvider = await fetch(`${PAYMENT_PROVIDER_URL}/health`);
    if (!healthProvider.ok) throw new Error(`Payment provider health check returned ${healthProvider.status}`);
  } catch (err) {
  ...
  const paymentEvents: PaymentProviderEvent[] = [];
  ...
  for (let i = 0; i < TOTAL_REQUESTS; i += creationBatchSize) {
    ...
    const batchPromises = Array.from({ length: batchCount }, async (_, idx) => {
      ...
      return (await res.json()) as PaymentProviderEvent;
    });
  ...
  const queryPlanner = explanation.queryPlanner as { winningPlan: { stage: string } };
  const executionStats = explanation.executionStats as { executionTimeMillis: number };
