...
export function isSafeTarget(uri: string): boolean {
  try {
...
export function isRehearsalBranch(branchName: string): boolean {
...
export function isRehearsalPR(pr: { title: string; headRef: string; number: number }): boolean {
  ...
  return (
    titleLower.startsWith('[opsroom rehearsal]') ||
...
export function restoreAgentsBaseline(
  agentsPath: string = path.join(process.cwd(), 'AGENTS.md'),
  fixturePath: string = path.join(process.cwd(), 'scripts', 'fixtures', 'AGENTS.md.canonical')
): { restored: boolean; wasModified: boolean } {
...
export async function stripExtraneousIndexes(): Promise<number> {
...
export async function resetPaymentProvider(
  providerUrl: string = PAYMENT_PROVIDER_URL,
  storeFilePath: string = process.env['PAYMENT_STORE_FILE'] ?? path.join(process.cwd(), 'data', 'payments.json')
): Promise<{ success: boolean; eventsCleared: boolean }> {
...
export async function cleanupGithubRehearsals(options?: {
  ...
  try {
    ...
    const prsData = (await prsRes.json()) as Array<{
      number: number;
      title: string;
      head: { ref: string };
    }>;
    ...
    const branchesData = (await branchesRes.json()) as Array<{ name: string }>;

    for (const branch of branchesData) {
      if (isRehearsalBranch(branch.name)) {
        const deleteUrl = `https://api.github.com/repos/${owner}/${repoName}/git/refs/heads/${encodeURIComponent(
          branch.name
        )}`;
